...
// #ARKREP_MODIFIED_CODE : Added override to CreateBelowWidgetControls() functions
	...
	TSharedPtr<SWidget> GetStartHereWidget() const;
	TSharedPtr<SWidget> GetNonExecutableInPIEWidget() const;
...
protected:
	void CreateMetadataWidgets(TSharedPtr<SVerticalBox> mainBox);
	TSharedRef<SButton> CreateShowMetadataButton() const;

	const FSlateBrush* GetMetadataImageBrush();

	EVisibility GetShowMetadataButtonVisibility() const;
	EVisibility GetMetadataVisibility() const;
	FReply ToggleMetadataVisibility() const;

protected:
	TSharedPtr<FSlateBrush> MetaDataImageBrush { nullptr };
// !#ARKREP_MODIFIED_CODE
