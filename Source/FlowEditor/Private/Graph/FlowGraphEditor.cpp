...
// #ARKREP_MODIFIED_CODE : Added 'start here' related command binding
	CommandList->MapAction(FlowGraphCommands.StartHere,
	                               FExecuteAction::CreateSP(this, &SFlowGraphEditor::OnSetStartHere),
	                               FCanExecuteAction::CreateStatic(&SFlowGraphEditor::CanEdit),
	                               FIsActionChecked(),
	                               FIsActionButtonVisible::CreateStatic(&SFlowGraphEditor::CanEdit)
	);

	CommandList->MapAction(FlowGraphCommands.CancelStartHere,
	                               FExecuteAction::CreateSP(this, &SFlowGraphEditor::OnCancelStartHere),
	                               FCanExecuteAction::CreateStatic(&SFlowGraphEditor::CanEdit),
	                               FIsActionChecked(),
	                               FIsActionButtonVisible::CreateStatic(&SFlowGraphEditor::CanEdit)
	);
...
void SFlowGraphEditor::OnSetStartHere() const
{
	const TSet<UFlowGraphNode*> selectedNodes = GetSelectedFlowNodes();
	if (selectedNodes.IsEmpty())
		return;

	// Only set start here of first selected node
	UFlowGraphNode* newStartNode = selectedNodes.Get(FSetElementId::FromInteger(0));
	if (newStartNode == nullptr)
		return;

	// Cancel start here of all node because only one can be set per graph
	if (UFlowGraph* graph = Cast<UFlowGraph>(GetCurrentGraph()))
	{
		for (UEdGraphNode* graphNode : graph->Nodes)
		{
			if (const UFlowGraphNode* flowGraphNode = Cast<UFlowGraphNode>(graphNode))
				flowGraphNode->CancelStartHere();
		}
	}

	newStartNode->SetStartHere();
	FlowAsset->Modify();
}

void SFlowGraphEditor::OnCancelStartHere() const
{
	const TSet<UFlowGraphNode*> selectedNodes = GetSelectedFlowNodes();
	if (selectedNodes.IsEmpty())
		return;

	for (const UFlowGraphNode* node : selectedNodes)
	{
		if (node != nullptr)
			node->CancelStartHere();
	}
