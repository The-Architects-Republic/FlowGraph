...
// #ARKREP_MODIFIED_CODE : Added 'Non-executable nodes' related code
		...
		if (GEditor != nullptr)
		{
			 ...
			 * to see if they should be marked as 'non-executable' */
			if (OutputPins.Num() > 1 && (ActivationState == EFlowNodeState::Aborted || ActivationState == EFlowNodeState::Completed))
			{
...
void UFlowNode::MarkOutputNodesAsNonExecutableInPIE(const int32 executedOutputIndex) const
{
	const UFlowAsset* graphAsset = GetFlowAsset();
	if (graphAsset == nullptr)
		return;

	const FFlowPin& executedOutput = OutputPins[executedOutputIndex];

	FGuid executedOutputNodeUid{};
	TSet<FGuid> connectedOutputNodesUid {};
	...
	for (const FFlowPin& pin : OutputPins)
	{
		if (pin.GetPinType() == EFlowPinType::Exec)
		{
			...
			{
				connectedOutputNodesUid.Add(connection.NodeGuid);

				if (pin == executedOutput)
					executedOutputNodeUid = connection.NodeGuid;
			}
		}
	}

	/* Ensure more than one node are connected.
	 * If not, there is no point of running this function because the solo connected output node will be executed. */
	if (connectedOutputNodesUid.Num() == 1)
		return;

	TArray<UFlowNode*> nextExecutedNodes {};

	// Is executed output connected to a node ?
	if (executedOutputNodeUid.IsValid())
	{
		if (UFlowNode* nextExecutedNode = GetFlowAsset()->GetNode(executedOutputNodeUid))
		{
			nextExecutedNodes = graphAsset->GetNodesInExecutionOrder(nextExecutedNode, UFlowNode::StaticClass());
		}
	}
	
	/* Check all connected output nodes (except the one from the executed output) to see if them
	 * and their linked nodes should be marked as non-executable */
	for (const FGuid& connectedOutputNodeUid : connectedOutputNodesUid)
	{
		if (connectedOutputNodeUid == executedOutputNodeUid)
			continue;

		UFlowNode* outputConnectedNode = GetFlowAsset()->GetNode(connectedOutputNodeUid);
		if (outputConnectedNode == nullptr)
			continue;

		// If output node is active, cancel here because its linked nodes can be executed
		if (outputConnectedNode->GetActivationState() == EFlowNodeState::Active)
			break;

		// Gather the nodes connected to the currently checked output node
		const TArray<UFlowNode*> connectedNodes = graphAsset->GetNodesInExecutionOrder(outputConnectedNode, UFlowNode::StaticClass());

		for (UFlowNode* node : connectedNodes)
		{
			// If node is active, cancel here because the linked nodes can be executed
			if (node->GetActivationState() == EFlowNodeState::Active)
				break;
			// If node was completed, do not mark it as non-executable
			if (node->GetActivationState() == EFlowNodeState::Completed)
				continue;

			// If the node is linked to one of the executed output node's connected node, stop the check and process the next output node, because this node can be executed
			if (!nextExecutedNodes.Contains(node))
			{
				node->SetNonExecutableInPIE(true);
			}
			else // process next connected output node
				break;
		}
	}
}

void UFlowNode::SetNonExecutableInPIE(const bool isNonExecutable)
{
	if (_IsNonExecutableInPIE == isNonExecutable)
		return;

	_IsNonExecutableInPIE = isNonExecutable;
}
