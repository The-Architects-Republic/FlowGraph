 ...
 * #ARKREP_MODIFIED_CODE : Added HideDropdown so it cannot be used to create an asset because we must only use our derived child classes.
 */
UCLASS(BlueprintType, HideDropdown, hideCategories = Object)
...
#if WITH_EDITOR
	UFlowNode* GetStartHereNode() const;
#endif
...
// #ARKREP_MODIFIED_CODE : Made ActiveInstances protected instead of private because we need to access it from the child class
protected:
