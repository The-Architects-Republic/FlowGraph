...
// #ARKREP_MODIFIED_CODE : Added metadata related include
...
protected:
#if WITH_EDITORONLY_DATA
	/** If true, the graph will try to start at this node, ignoring its 'Start' node. (Editor only) */
	UPROPERTY(DuplicateTransient)
	bool GraphStartHere { false };

	/** Editor metadata to be displayed on the graph node */
	UPROPERTY(EditAnywhere, meta=(ShowOnlyInnerProperties), Category="Metadata")
	FFlowNodeEditorMetaData EditorMetadata {};
#endif
...
#if WITH_EDITOR
public:
	bool IsNonExecutableInPIE() const { return _IsNonExecutableInPIE; }

protected:
#if WITH_EDITORONLY_DATA
	/**
	 * PIE only: Is true if this node will never be executed while playing in the editor due to the graph's current flow.
	 * This will change the node's widget appearance.
	 */
	UPROPERTY(Transient)
	bool _IsNonExecutableInPIE { false };
#endif

protected:
	/**
	 * PIE only: Gather all nodes connected to this node instance's outputs and mark them as 'non-executable'
	 * if they cannot be executed due to the graph's current flow.
	 * The nodes after the 'executedOutputIndex' will never be marked as non-executable because they are the next that are going to be activated.
	 * @param executedOutputIndex the output index of the node that just finished. All nodes connected to the other outputs will be checked to be marked as non-executable.
	 */
	void MarkOutputNodesAsNonExecutableInPIE(const int32 executedOutputIndex) const;
	void SetNonExecutableInPIE(const bool isNonExecutable);
...
// #ARKREP_MODIFIED_CODE : Added GetActorsToFocus() function
	/** Return a list of actors this node is referencing */
	virtual TSet<AActor*> GetActorsToFocus() { return {}; }

// #ARKREP_MODIFIED_CODE : Added SetResourceObject() function
	/**
	 * Set the resource object (asset) referenced and used by this node.
	 * This is the asset that must be returned by the GetAssetToEdit() function.
	 */
	virtual void SetResourceObject(UObject* resource) PURE_VIRTUAL(SetResourceObject);
#endif
